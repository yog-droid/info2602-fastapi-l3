...
def print_todos():
    ...
    with get_session() as db:
        ...
        for todo in todos:
            username = todo.user.username if todo.user else "(no user)"
            print(f"{todo.id} - {todo.text} - {username} - Done: {todo.done}")
    ...
    with get_session() as db:
        ...
        for todo in todos:
            if not todo.done:
                ...
                count += 1
        db.commit()
        print(f"Marked {count} todos complete for {username}")
